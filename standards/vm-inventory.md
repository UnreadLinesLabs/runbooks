...
| `U01PARVMNPS01` | Network Policy Server (RADIUS) | Subnet 1 (`192.168.20.0/25`) | `192.168.20.46/25` | `192.168.20.126` | Windows Server | Yes | Planned | RADIUS for SSID#2 (802.1X EAP-TLS); RADIUS client is `RAP01` (Subnet 2), reachable via the FWL01↔FWL02 WireGuard tunnel; NPS server certificate pending — see the `radius-nps-deployment` lab |
...
| `U01PARVMRAP01` | Wireless Access Point (`hostapd`, USB Wi-Fi passthrough) | Subnet 2 (`192.168.20.128/25`); second NIC on `VMnet3` | `192.168.20.145/25` (management, `ens33`); no IP on `ens37`/`VMnet3` | `192.168.20.254` | Ubuntu/Debian | No | Running | SSID#1 (`UnreadLines-Guest`, WPA2-PSK + DHCP) validated end-to-end: association, DHCP lease, Internet. Bridges the Wi-Fi radio to `VMnet3` (`br-vlan10`, member `ens37`) — a dedicated VMware network, not a VLAN trunk. SSID#2 (802.1X EAP-TLS) pending the NPS server certificate — see the hostapd Wi-Fi AP lab and the `radius-nps-deployment` lab |
...
| `VMnet4` (planned) | SSID#2 — WPA2/WPA3-Enterprise (802.1X EAP-TLS) | `192.168.21.128/25` | `FWL02` (new interface, not yet created) | Reserved, not yet active |
...
| SSID#2 / `VMnet4` (`192.168.21.128/25`, reserved) | `.129`–`.254` | static range `.129`–`.137`; DHCP pool `.178`–`.228` (reserved, not yet built) |
