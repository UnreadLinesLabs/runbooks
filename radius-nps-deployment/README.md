...
This lab deploys `U01PARVMNPS01`, a dedicated Windows Server running the **Network Policy Server (NPS)** role — Microsoft's RADIUS implementation. It will authenticate `UnreadLines-Mobile`, the WPA2/WPA3-Enterprise (802.1X EAP-TLS) SSID that `hostapd-wifi-access-point/README.md` §15 plans for domain-managed, Intune-enrolled mobile devices — the first of two enterprise SSIDs on that naming plan (`UnreadLines-Corp`, for domain-joined workstations, comes later and is out of scope here). §15 defers building it to "once RADIUS/NPS exists" — that server is `U01PARVMNPS01`, built here.
...
```text
      Subnet 2 - 192.168.20.128/25              Subnet 1 - 192.168.20.0/25
   +------------------------------+          +------------------------------+
   |  U01PARVMRAP01               |          |  U01PARVMNPS01               |
   |  192.168.20.145              |          |  192.168.20.46               |
   |  hostapd AP = RADIUS client  |          |  NPS = RADIUS server         |
   +---------------+--------------+          +---------------+--------------+
                   |                                         |
            U01PARVMFWL02 ===== WireGuard tunnel ===== U01PARVMFWL01
            192.168.20.254                             192.168.20.126
                   |                                         |
                   +---- RADIUS  UDP 1812 / 1813 ------------+
                                                     ...
                                                     192.168.20.41
                                                     AD DS / DNS
                                                     GG-U01-PAR-WiFi-Mobile
```

The RADIUS client and the RADIUS server sit in different subnets on different physical hosts. That is
deliberate, not an oversight: the tunnel built in `openwrt-wireguard-site-to-site/README.md` already
routes between them, so no extra firewall or routing work is needed here.
...
| Target SSID | `UnreadLines-Mobile` (WPA2/WPA3-Enterprise, 802.1X EAP-TLS) — see `hostapd-wifi-access-point/README.md` §15 |
| Wi-Fi access group | `GG-U01-PAR-WiFi-Mobile` |

`RAP01` sits in Subnet 2 (`192.168.20.128/25`) while NPS sits in Subnet 1 — this is intentional, not an oversight. `openwrt-wireguard-site-to-site/README.md` already establishes and verifies full routing between the two LAN subnets over the FWL01↔FWL02 WireGuard tunnel (§22, "Full end-to-end verification"), so RADIUS traffic from `RAP01` reaches `NPS01` the same way any other cross-subnet traffic already does. No additional OpenWrt routing or firewall change is required: the existing `lan ↔ wg` forwarding rules on both routers already accept it.

## 4. Prerequisites

- `U01PARVMDOM01` (AD DS/DNS) is up and reachable.
- The FWL01↔FWL02 WireGuard tunnel is up and verified (`openwrt-wireguard-site-to-site/README.md`, §21–22).
...
Reconnect after the restart using a domain account with local Administrator rights on this server and enough AD rights to modify the built-in `RAS and IAS Servers` group (Domain Admins, or an explicit delegation) — needed for the registration step in §7. `GG-U01-PAR-WiFi-Mobile` (§8) is an access-control group for Wi-Fi clients, not an administrative delegation — it grants nothing here. A local (pre-domain-join) `Administrator` session is not sufficient beyond this point.
...
## 6. Install the Network Policy Server role
...
`NPAS` does not include the AD PowerShell module (`Get-ADGroupMember`, `New-ADGroup`, etc.) — don't install `RSAT-AD-PowerShell` here just to get it. `U01PARVMDOM01` already has the `ActiveDirectory` module natively, as a domain controller, so every AD-native command in this lab (§7, §8, §13) is run there instead — the same split `ad-cs-pki-deployment/README.md` uses for its own GPO steps ("Switch servers for this part — the `GroupPolicy` module isn't necessarily installed on `U01PARVMPKI02`. Use `U01PARVMDOM01`.").

## 7. Register NPS in Active Directory
...
Leave the group empty for now — there is no real `UnreadLines-Mobile` user or device to add yet. Membership only starts mattering once a pilot mobile is actually enrolled in Intune and needs Wi-Fi access, which is later work, not part of this lab. When that day comes, adding a member looks like this — replace `<SamAccountName>` with a real existing account or computer name, never run it with a placeholder value:

```powershell
Add-ADGroupMember -Identity "GG-U01-PAR-WiFi-Mobile" -Members "<SamAccountName>"
```

## 9. Add the RADIUS client (NAS)

`hostapd` on `U01PARVMRAP01` — not `FWL02` — is the RADIUS client: it is the process that will send Access-Request packets directly to NPS once `UnreadLines-Mobile` is built and active (`hostapd-wifi-access-point/README.md` §15). Generate a strong shared secret and store it in a password vault before this step; it will be needed again in `hostapd.conf`.
...
## 10. Leave the default Connection Request Policy
...
## 11. Create the Network Policy for `UnreadLines-Mobile`

Under `Policies`, NPS has two similarly-named nodes — **Connection Request Policies** and **Network Policies**. This step is the second one. §10 already confirmed the default Connection Request Policy needs no changes; don't create a new Connection Request Policy here.
...
RADIUS Attributes → Standard        : not needed. This is where Tunnel-Type /
                                       ...
                                       per client — but hostapd-wifi-access-point
                                       abandoned per-client VLANs entirely (§11): each
...
RADIUS Attributes → Vendor Specific : not needed — RAP01 was declared with vendor
                                       "RADIUS Standard" in §9, not a vendor requiring
                                       custom VSAs.
...
## 12. Open the Windows Firewall for RADIUS
...
By default `radtest` sends `NAS-IP-Address` as whatever the local machine's hostname resolves to (`127.0.1.1` on a typical Ubuntu `/etc/hosts` — not `RAP01`'s real management IP), which won't match this policy's `NAS IPv4 Address` condition (§11). Pass `RAP01`'s real address explicitly as the trailing `nasname` argument:
...
- **No reply at all (timeout)** → the shared secret doesn't match on one side, or the request never reached NPS (routing/firewall). Check `netsh nps show config` and the secret configured in §9.
...
- **`Access-Reject`, and the NPS Security event log names the reason** — check it on `U01PARVMNPS01`:
  ...
  User → Security ID              : S-1-5-21-... (a real SID — not S-1-0-0 / NULL SID,
                                     the specific failure an earlier NPS attempt hit
...
3. Since `UnreadLines-Mobile` is EAP-TLS end-to-end (client certificates too, not just the server side), issue a client-authentication certificate to each device or user permitted in `GG-U01-PAR-WiFi-Mobile` — via Intune/SCEP for these Intune-enrolled mobile devices.
4. Build `UnreadLines-Mobile` itself and switch `RAP01` over to it by following `hostapd-wifi-access-point/README.md` §15: create the new dedicated VMware network (e.g. `VMnet4` — not a VLAN, see §11), wire it into `RAP01`'s Netplan and `FWL02`, then stop `hostapd` and edit `hostapd.conf` to `ssid=UnreadLines-Mobile`, `ieee8021x=1` / `wpa_key_mgmt=WPA-EAP` with `auth_server_addr=192.168.20.46`, the shared secret from §9 above, and `bridge=` set to that new network's bridge (replacing `br-vlan10`), then restart. `RAP01`'s single radio means `UnreadLines-Guest` goes offline while `UnreadLines-Mobile` is active.
