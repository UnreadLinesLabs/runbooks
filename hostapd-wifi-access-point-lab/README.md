...
| Wi-Fi hardware | Alfa AWUS1900 (Realtek RTL8814AU), USB passthrough — single BSS only, see §3.4 |
| SSID#1 | `UnreadLines-Guest`, WPA2-Personal |
| SSID#1 client pool | `192.168.21.50`–`192.168.21.99`, 12h lease |

Update `standards/vm-inventory.md` once this is deployed, if any of these values change.

## Prerequisites

- `U01PARVMFWL02` already deployed and routing Subnet 2 (see `openwrt-wireguard-site-to-site-lab/README.md`).
- An Alfa AWUS1900 (Realtek RTL8814AU) USB Wi-Fi adapter, with AP mode confirmed via `iw list` (§3.4).
...
- `192.168.20.145` and `192.168.21.0/25` confirmed free (checked against `standards/vm-inventory.md` §4 at time of writing).

## 3. Prepare the Ubuntu/Debian VM (`RAP01`)
...
3. Start the VM, then in VMware Workstation: **VM → Removable Devices → \<adapter name\> → Connect (Disconnect from Host)**.
   If SSH isn't reachable after this boot (common right after cloning a template), reconnect via the VMware console and run §3.1 first.
...
### 3.4 Driver — the in-kernel driver is enough
...
Reconnect the USB device (§3.3), then confirm `lsmod | grep 8814au` and `iw dev` show the driver and interface.

### 3.5 Network configuration (Netplan)

`RAP01` needs two things configured here: its existing management NIC (`ens33`, unchanged from before), and the new `VMnet3` NIC (`ens37`) enslaved to a bridge that `hostapd` will also attach the Wi-Fi radio to. Add the `VMnet3` adapter to the VM first (§4) — `ens37` won't exist until you do.
...
network:
  ...
  bridges:
    br-vlan10:
      ...
      # RAP01 doesn't need an address here to do its job (§9 / Security).
      addresses: [192.168.21.2/25]
```

`br-vlan10` is just a name (kept from the earlier VLAN design, §9) — it's a plain, untagged bridge. `ens37` has no IP of its own; the bridge is the Layer 2 meeting point between `ens37` (towards `FWL02`) and `wlx00c0cab23594` (towards the Wi-Fi client), which `hostapd` attaches automatically when it starts (§5).
...
## 4. Create `VMnet3` in VMware Workstation
...
DHCP is disabled because `FWL02` is the only DHCP server for this segment (§6.3). The host adapter is disabled to keep Windows off the Wi-Fi segment entirely and avoid address conflicts.
...
## 5. Install and configure `hostapd`
...
`driver=nl80211` tells `hostapd` to control the radio through the kernel's modern netlink-based wireless API, rather than the old, deprecated `wext` interface. It's the value used with any `mac80211`-based driver, which is exactly what `rtw88_8814au` (§3.4) is — so this is effectively the only correct choice here, not a pick between alternatives. `country_code=FR` sets the regulatory domain on every start, independent of the driver (§3.4).
...
## 6. Configure `U01PARVMFWL02`: Wi-Fi interface, firewall, DHCP

SSH to `FWL02` (`192.168.20.254`). All of this is on `eth2` (§4) — no VLAN subinterface, no trunk.
...
`input='ACCEPT'` makes testing against `192.168.21.1` easy for the lab; harden it once validated (§12).
...
**Phone won't associate to the SSID at all** — confirm `hostapd` is actually up and `AP-ENABLED` (§5), then check for a stuck association:
...
Rather than keep chasing a VMware Workstation internals issue, the design was changed to two separate host-only networks instead of one trunk — `VMnet2` for the existing LAN, `VMnet3` dedicated to the Wi-Fi segment (§4). Untagged Ethernet between VMs on the same host-only network is exactly what Workstation is built to do well, and it's what this revision of the lab uses throughout.
...
* [ ] `standards/vm-inventory.md` updated with the real IPs/status now that this is deployed.
