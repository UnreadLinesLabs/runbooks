# Issue the NPS server certificate and activate EAP-TLS for `UnreadLines-Mobile`
...
```text
  ...
  U01PARVMPKI02   Issuing CA          192.168.20.143
        |  issues a "Server Authentication" certificate
        v
  U01PARVMNPS01   NPS / RADIUS        192.168.20.46
        |  presents that certificate during the EAP-TLS handshake
        v
  Wi-Fi client    joining UnreadLines-Mobile
        |
        +-- trusts the Root CA, and presents a client certificate of its own
            (delivering that client certificate is not covered here)

  U01PARVMDOM01   AD DS / DNS         192.168.20.41
        autoenrolment GPO — keeps the NPS certificate renewing on its own
```

## 2. Mutual authentication in EAP-TLS
...
Mobile device verifies:
    ...
    - name matches the server it's connecting to (`U01PARVMNPS01`'s SAN — see §7's Subject Name tab)?
    |
    v
...
## 3. Where this lab fits

The certificate and RADIUS chain
...
failure mode that an earlier, undocumented attempt ran into, but nothing in this lab or the next few
...
(`hostapd-wifi-access-point/README.md` §15), and a real 802.1X handshake confirmed in the NPS event log.
...
| Bound to | Network Policy `UnreadLines-Mobile - EAP-TLS` (`radius-nps-deployment/README.md` §11) |
...
- The Network Policy `UnreadLines-Mobile - EAP-TLS` already exists with `Microsoft: Smart Card or other
  certificate` as its only authentication method (`radius-nps-deployment/README.md` §11) — it just has nothing to
  bind yet.
- Administrative access on both `U01PARVMPKI02` and `U01PARVMNPS01`.

## 7. Publish the Server Authentication template — on `U01PARVMPKI02`
...
§7, `netsh ras add registeredserver`) already made it a member of the built-in `RAS and IAS Servers` group, and
...
`NotAfter`, with no one having to repeat §8 manually.
...
`Certificate Services Client - Auto-Enrollment`, scoped to `U01PARVMNPS01` — built in §11 below, after the
certificate itself is enrolled and bound. Until §11 is done, granting `Autoenroll` here is necessary groundwork,
...
## 8. Enroll the certificate — on `U01PARVMNPS01`
...
thumbprints — leaving two entries to disambiguate for no reason in §9.

This is the certificate's first, manual issuance — §7 granted `Autoenroll` for *renewals*, it doesn't skip this
initial request, and the GPO that actually makes autoenrollment happen isn't built until §11, after this
certificate already exists. (Re-running this lab later — say, after a revocation — is a different story: at that
point the GPO from §11 already exists and applies, so `U01PARVMNPS01` could pick the certificate back up on its
...
account's `Enroll` right from §7 didn't take — re-check the Security tab on `U01PARVMPKI02` before assuming a
client-side problem.

If both ended up running anyway, nothing is broken — both certificates are equally valid, just redundant. Compare
`NotBefore` to find the one just issued and bind that one in §9:
...
## 9. Bind the certificate to the Network Policy — on `U01PARVMNPS01`
...
nps.msc → Policies → Network Policies → UnreadLines-Mobile - EAP-TLS → Properties
    ...
    → Certificate issued to: <the NPS Server Authentication certificate enrolled in §8>
    → OK → Apply
```

The **Edit...** dialog only lists certificates in `LocalMachine\My` that carry the Server Authentication EKU and
a private key — if more than one shows up, match it by the Subject/Thumbprint confirmed in §8. Restart the NPS
...
## 11. Configure autoenrollment — on `U01PARVMDOM01`

`Autoenroll: Allow` on the template (§7) only grants `U01PARVMNPS01` the *right* to autoenroll — nothing polls for
...
the template's own ACL (§7) restricts enrollment to that one server rather than to `RAS and IAS Servers` at
large.
...
certificate already enrolled in §8 is still recognized (this is also what a real renewal will look like, later,
closer to `NotAfter`):
...
templates `U01PARVMNPS01` currently has `Autoenroll` rights on (via AD, per §7's ACL) and whether a valid
...
⚠️ Run this against the **duplicate** certificate from §8 if one still exists, not the one bound in `nps.msc`
(§9) — deleting the bound certificate breaks `UnreadLines-Mobile - EAP-TLS`'s EAP-TLS binding immediately, since
`nps.msc` doesn't automatically rebind to a replacement; that would mean redoing §9 by hand afterward just to
...
§8 manually unless the certificate is revoked or the template changes.
...
certificates to permitted devices via Intune/SCEP, build
`UnreadLines-Mobile` and switch `RAP01` over from `UnreadLines-Guest`
(`hostapd-wifi-access-point/README.md` §15), and confirm a real `Access-Accept` in the NPS event log.
Those land across the videos that follow, after the SCEP user
