...
| `U01PARVMNPS01` | Network Policy Server (RADIUS) | Subnet 1 (`192.168.20.0/25`) | `192.168.20.46/25` | `192.168.20.126` | Windows Server | Yes | Running | RADIUS for `UnreadLines-Mobile` (802.1X EAP-TLS); RADIUS client is `RAP01` (Subnet 2), reachable via the FWL01↔FWL02 WireGuard tunnel; NPS server certificate issued and bound to the `UnreadLines-Mobile - EAP-TLS` Network Policy — see the `radius-nps-deployment` and `nps-server-certificate-deployment` labs, both published |
...
| `U01PARVMRAP01` | Wireless Access Point (`hostapd`, USB Wi-Fi passthrough) | Subnet 2 (`192.168.20.128/25`); second NIC on `VMnet3` | `192.168.20.145/25` (management, `ens33`); no IP on `ens37`/`VMnet3` | `192.168.20.254` | Ubuntu/Debian | No | Running | SSID#1 (`UnreadLines-Guest`, WPA2-PSK + DHCP) validated end-to-end: association, DHCP lease, Internet. Bridges the Wi-Fi radio to `VMnet3` (`br-vlan10`, member `ens37`) — a dedicated VMware network, not a VLAN trunk. `UnreadLines-Mobile` (802.1X EAP-TLS) pending a client certificate — the NPS server certificate itself is issued (`nps-server-certificate-deployment`, published) — see the hostapd Wi-Fi AP lab §15 and the `radius-nps-deployment` lab |
| `U01PARVMECN01` | Microsoft Entra Connect Sync | Subnet 2 (`192.168.20.128/25`) | `192.168.20.146/25` | `192.168.20.254` | Windows Server | Yes | Running | Hybrid identity sync for `corp.unreadlines.com` ↔ Entra tenant `unreadlines`, Password Hash Sync, OU-filtered to standard accounts (`u...`) only — see the `microsoft-entra-connect-sync` lab, published |
| `U01PARVMNDS01` | Network Device Enrollment Service (NDES) + Intune Certificate Connector | Subnet 2 (`192.168.20.128/25`) | `192.168.20.147/25` | `192.168.20.254` | Windows Server | Yes | Running | SCEP endpoint for Intune-managed mobile certificates; Intune Certificate Connector installed on this same host — required by Microsoft when SCEP uses a local CA, not a choice. Briefly reserved as `U00PARVMNDS01` (`naming-conventions.md` §1), reverted to `U01PARVMNDS01` on 2026-09-09 once its runbook and real build were already underway. **Build completed 2026-09-09** — `Intune SCEP Mobile User` template published on `U01PARVMPKI02`, NDES role configured and running under `gmsa-ndes$`, Certificate Connector installed, registered, and confirmed **Active** in the tenant (`ndes-scep-intune-connector/README.md` §6–§12 done). The connector registers under an Intune-generated default name unrelated to the hostname (`PFX_connector_<date/time>`) rather than under `U01PARVMNDS01` — renamed to `SCEP_PFX_U01PARVMNDS01_connector_<date/time>` for recognizability; its enabled features (`SCEP` only, confirmed on the host, §12) don't follow from that name — see backlog item 4 (`YouTube/backlog.md` §2) and `notes/wifi-mobile-certificate-chain.md` §6. |
| `U00PARVMPNC01` | Microsoft Entra Private Network Connector | Subnet 2 (`192.168.20.128/25`) | `192.168.20.148/25` | `192.168.20.254` | Windows Server | No | Running | Deliberately on a separate host from `U01PARVMNDS01`; publishes the NDES SCEP endpoint through Entra Application Proxy (Passthrough preauth) without exposing it directly or opening an inbound port. `U00` (not `U01`), per `naming-conventions.md` §1: this connector is company-wide infrastructure, hosted in Paris but not France-specific — renamed before any runbook existed for it. **Build completed 2026-09-12** — Private Network Connector installed and confirmed **Active** in the tenant; not domain-joined, so the `UnreadLines Root CA`/`UnreadLines Issuing CA` trust was installed manually from `C:\UnreadLines\`. Publishes `App Proxy - NDES SCEP - Mobile` at `https://AppProxyNDESSCEPMobile-unreadlines.msappproxy.net` (Passthrough preauth, admin consent for `User.Read` granted). NDES's IIS site (`U01PARVMNDS01`) carries a URL Rewrite blocking rule allowing only `mscep.dll`/`pkiclient.exe` and `CertificateRegistrationSvc`; end-to-end SCEP enrollment verified from a real mobile device over cellular data (`entra-private-network-connector-app-proxy/README.md` §9–§13 done). See backlog item 5 (`YouTube/backlog.md` §2) and `notes/wifi-mobile-certificate-chain.md`. |
...
| Subnet 2 (`192.168.20.128/25`) | `.129`–`.254` | `.149` |
