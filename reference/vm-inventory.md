...
| `U01PARVMNDS01` | Network Device Enrollment Service (NDES) + Intune Certificate Connector | Subnet 2 (`192.168.20.128/25`) | `192.168.20.147/25` | `192.168.20.254` | Windows Server | Yes | Planned | SCEP endpoint for Intune-managed mobile certificates; Intune Certificate Connector installed on this same host — required by Microsoft when SCEP uses a local CA, not a choice. See backlog item 4 (`YouTube/backlog.md` §2) and `notes/wifi-mobile-certificate-chain.md`. |
| `U01PARVMPNC01` | Microsoft Entra Private Network Connector | Subnet 2 (`192.168.20.128/25`) | `192.168.20.148/25` | `192.168.20.254` | Windows Server | No (to confirm at install) | Planned | Deliberately on a separate host from `U01PARVMNDS01`; publishes the NDES SCEP endpoint through Entra Application Proxy (Passthrough preauth) without exposing it directly or opening an inbound port. See backlog item 5 (`YouTube/backlog.md` §2) and `notes/wifi-mobile-certificate-chain.md`. |
