...
**A `Synced` sub-OU under any `Groups` OU marks the specific groups that reach Entra ID.** Microsoft Entra Connect's OU-based sync filtering (`microsoft-entra-connect-sync/README.md` §8) selects whole OUs, not individual objects — syncing a `Groups` OU wholesale would put every `ADM`/tier-0 group in that OU on the same list as ordinary workload groups, breaking the tier isolation this section already builds into the tree. The fix is a dedicated `OU=Synced` sub-OU created only where at least one group actually needs to reach Entra ID, holding just that group (or those groups) and nothing else — every other group under the parent `Groups` OU, `ADM` groups included, stays out of Entra Connect's selection by construction, not by a promise to remember excluding them. First real instance: `OU=Synced,OU=Groups,OU=PAR,OU=U01,OU=UnreadLines,DC=corp,DC=unreadlines,DC=com`, holding `GG-U01-PAR-WiFi-Mobile` (`intune-scep-certificate-profile/README.md` §5–§6) — created the moment Intune needed to target that group's synced membership, not proactively. The same pattern applies at any level of the tree (`OU=U00/Groups`, `OU=U01/Groups`, or any city's) the moment a group at that level needs the same treatment; there is no single company-wide `Synced` sub-tree — each `Groups` OU gets at most one of its own.
